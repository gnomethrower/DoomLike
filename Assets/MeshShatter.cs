...
public class MeshShatter : MonoBehaviour
{
    ...
    public float partLifespan = 5f;
    public int CutCascades = 1;
    public float ExplodeForce = 0;

    Mortality_Script mortalityScript;

    // Start is called before the first frame update
    void Start()
    {
        mortalityScript = gameObject.GetComponent<Mortality_Script>();
    }

    // Update is called once per frame
    void Update()
    {
        if (mortalityScript != null)
        {
            if (mortalityScript.currentHealth <= 0)
            {
                DestroyMesh();
            }
        }
    ...
    private void DestroyMesh()
    {
        ...
        for (var c = 0; c < CutCascades; c++)
        {
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                StartCoroutine(DestroyPartAfterDelay(parts[i].GameObject, partLifespan));
            }
    ...
    private IEnumerator DestroyPartAfterDelay(GameObject gameObject, float delay)
    {
        yield return new WaitForSeconds(delay);
        Debug.Log("Destroying parts");
        Destroy(gameObject);
    }
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshShatter original)
        {
            ...
            var meshShatter = GameObject.AddComponent<MeshShatter>();
            meshShatter.CutCascades = original.CutCascades;
            meshShatter.ExplodeForce = original.ExplodeForce;
